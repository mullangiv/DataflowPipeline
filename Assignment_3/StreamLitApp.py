...
chosenRadioButton = st.sidebar.radio(
    ...
    ("HomePage","Authentication","ScrapeData","IdentifyEntities","Anonymize","De/Re-Identify","Sentiment")
)
...
if chosenRadioButton == 'HomePage':
    st.image("welcome.gif", caption='',use_column_width=True)
...
if chosenRadioButton == 'ScrapeData':
    ...
    if st.button('Scrape Data'):
        ...
        response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/scrapeCallTranscripts?verified={authValue}&url={URL}&page={Page}")
        data_list = response.json()            
        st.subheader(data_list)

    st.subheader('_View Scrapped File List_')
    if st.button('Display Scraped File List'):
        response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/displayScrapedFilesList?verified={authValue}")
        data_list = response.json()
...
if chosenRadioButton == 'IdentifyEntities':
    ...
    if box == "Entities":
        ...
        if st.button('Entities'):
            ...
            #response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/identifyEntities?verified={authValue}&fileName={fileName}")
            data_list = response.json()
            st.header(data_list)
    else:
        st.title('Identify Keyphrases')
        st.subheader('_Please click the button to Identify PII entities_')
        if st.button('Keyphrases'):
            response = requests.get(f"http://localhost:8000/identifyKeyphrases?verified={authValue}&fileName={fileName}")
            #response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/identifyKeyphrases?verified={authValue}&fileName={fileName}")
            data_list = response.json()
            st.header(data_list)
...
if chosenRadioButton == 'Anonymize':
    ...
    if box == 'Masking':
        ...
        if st.button('Masking'):
            ...
            #response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/maskEntities?verified={authValue}&fileName={fileName}&entityList={entityList}&maskCharacter={maskCharacter}")
            data_list = response.json()
            jobID = data_list
            st.header(data_list)
        ...
        if st.button('View Masked Entities'):
            ...
            #response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/getMaskedEntities?verified={authValue}&jobID={jobID}&fileName={fileName}")
            data_list = response.json()
            prog = str(data_list)
        ...
        if st.button('Replace with EntityType'):
            ...
            #response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/replaceEntities?verified={authValue}&fileName={fileName}&entityList={entityList}")
            data_list = response.json()
            jobID = data_list
            st.header(data_list)
            st.success("Job Initated please use the job id to download the Masked File")  
        ...
        if st.button('View data Replaced with EntityType'):
            response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/displayEntities?verified={authValue}&jobID={jobID}&fileName={fileName}")
            data_list = response.json()
            prog = str(data_list)
            st.spinner(text='In progress...')
            st.success(data_list)
...
if chosenRadioButton == 'De/Re-Identify':
    ...
    if box == 'De-Identify':
        ...
        if st.button('De-Identify'):
            ...
            #response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/deIdentifyEntities?verified={authValue}&fileName={fileName}&JobName={jobName}")
            data_list = response.json()
            #b=data_list['body']
            st.header(data_list)
    else:
        ...
        drop = st.selectbox('Please select Entities for De-Identification',('Auto', 'Select'))
        st.write('You selected:', drop)
        #
        if drop == 'Auto':
            Auto = st.checkbox("Auto", value=False, key=None) #use ur brain bro make this a drop
            if Auto:
                entityList = "Auto"
                entityList = '"'+entityList+'"'
                if st.button('Re-Identify'):
                    response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/reIdentifyEntities?verified={authValue}&Hash={Hash}")
                    data_list = response.json()
                    #b=data_list['body']
                    st.header(data_list)
        else:
            entityList = ""
            st.info('Please select the Entity List for Re-Identification') #use this info for the drop and select option
            # rest in the drop select
            CREDIT_DEBIT_NUMBER = st.checkbox("CREDIT_DEBIT_NUMBER", value=False, key=None)
            if CREDIT_DEBIT_NUMBER:
                entityList = entityList+"CREDIT_DEBIT_NUMBER"+","
            ...
            condition2 = "BANK"
            condition3 = "PASSPORT"
            condition4 = "CREDIT_DEBIT"
            condition5 = "ACCESS"
            if condition1 in entityList or condition2 in entityList or condition3 in entityList or condition4 in entityList or condition5 in entityList :
                st.error(f"Please remove these from your selection: {condition1},{condition2},{condition3},{condition4},{condition5} ")
            else:
                if st.button('Re-Identify'):
                    response = requests.get(f"https://gbqrkn96z7.execute-api.us-east-1.amazonaws.com/prod/reIdentifyEntities?verified={authValue}&Hash={Hash}")
                    data_list = response.json()
...
if chosenRadioButton == 'Authentication':
    ...
    if st.button('Authenticate'):
        ...
        f.close()  
        print(authValue)
#Radio Button Condition 6
if chosenRadioButton == 'Sentiment':
    st.subheader('_Please provide text for sentiment analysis_')
    text = st.text_input('Text')
    data = {"text": text}
    data = json.dumps(data)
    if st.button('Predict'):
        if AuthStatus.Status == True:
            response = requests.post(url ="http://127.0.0.1:9000/predict", data = data)
            data_list = response.json()
            print(f"value is : {data_list}")
            st.success(data_list) 
        else:
            st.error("Authenticate Yourself")
            st.info("Please retry with valid Username and Password Combination")
