...
async def get_open_api_endpoint(currentUser: CognitoClaims = Depends(getUser)):
...
async def get_documentation(currentUser: CognitoClaims = Depends(getUser)):
...
async def generateRefreshToken_user_on_cognito(usrName: str, usrPassword: str, api_key: APIKey = Depends(get_api_key)): 
    OTP = usrName+usrPassword
    dynamodb = boto3.resource('dynamodb')
    table = dynamodb.Table('Users')
    response = table.get_item(Key = {'Login': OTP})
...
async def anonymize_data_using_faker(sourceTableName: str, newTableName: str, currentUser: CognitoClaims = Depends(getUser)):
    ...
    for user in Users:
        user['Username']['S'] = faker.name()   
        
        response = updateTable.put_item(
           Item={'Login': user['Login']['S'],
                  'Password': user['Password']['S'],
                   'RefreshJWT': user['RefreshJWT']['S'],
                     'SudoName': user['Username']['S']
                    }
        )
